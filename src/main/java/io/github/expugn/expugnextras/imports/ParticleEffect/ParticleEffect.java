package io.github.expugn.expugnextras.imports.ParticleEffect;
...
import io.github.expugn.expugnextras.imports.ParticleEffect.ReflectionUtils.PackageType;
...
public enum ParticleEffect {
	...
	EXPLOSION_NORMAL("explode", 0, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed by exploding ghast
											 * fireballs and wither skulls:
											 * <ul>
											 * <li>It looks like a gray ball
											 * which is fading away
											 * <li>The speed value slightly
											 * influences the size of this
											 * particle effect
											 * </ul>
											 */
	EXPLOSION_LARGE("largeexplode", 1,
			-1), /**
					 * A particle effect which is displayed by exploding tnt and
					 * creepers:
					 * <ul>
					 * <li>It looks like a crowd of gray balls which are fading
					 * away
					 * <li>The speed value has no influence on this particle
					 * effect
					 * </ul>
					 */
	EXPLOSION_HUGE("hugeexplosion", 2,
			-1), /**
					 * A particle effect which is displayed by launching
					 * fireworks:
					 * <ul>
					 * <li>It looks like a white star which is sparkling
					 * <li>The speed value influences the velocity at which the
					 * particle flies off
					 * </ul>
					 */
	FIREWORKS_SPARK("fireworksSpark", 3, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed by swimming entities
											 * and arrows in water:
											 * <ul>
											 * <li>It looks like a bubble
											 * <li>The speed value influences
	...
	WATER_BUBBLE("bubble", 4, -1, ParticleProperty.DIRECTIONAL,
			ParticleProperty.REQUIRES_WATER), /**
												 * A particle effect which is
												 * displayed by swimming
												 * entities and shaking wolves:
												 * <ul>
												 * <li>It looks like a blue drop
												 * <li>The speed value has no
												 * influence on this particle
												 * effect
												 * </ul>
												 */
	...
	WATER_WAKE("wake", 6, 7,
			ParticleProperty.DIRECTIONAL), /**
											 ...
											 * displayed by water:
											 * <ul>
											 * <li>It looks like a tiny blue
											 * square
											 * <li>The speed value has no
											 * influence on this particle effect
											 * </ul>
											 */
	SUSPENDED("suspended", 7, -1,
			ParticleProperty.REQUIRES_WATER), /**
												 * A particle effect which is
												 * displayed by air when close
												 * to bedrock and the in the
												 * void:
												 * <ul>
												 * <li>It looks like a tiny gray
												 * square
												 * <li>The speed value has no
												 * influence on this particle
												 * effect
	...
	SUSPENDED_DEPTH("depthSuspend", 8, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed when landing a critical
											 * hit and by arrows:
											 * <ul>
											 * <li>It looks like a light brown
											 * cross
											 * <li>The speed value influences
											 * the velocity at which the
											 * particle flies off
	...
	CRIT("crit", 9, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed when landing a hit with
											 * an enchanted weapon:
											 * <ul>
	...
	CRIT_MAGIC("magicCrit", 10, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed by primed tnt, torches,
											 * droppers, dispensers, end
											 * portals, brewing stands and
											 * monster spawners:
											 * <ul>
											 * <li>It looks like a little gray
											 * cloud
											 * <li>The speed value influences
											 * the velocity at which the
											 * particle flies off
	...
	SMOKE_NORMAL("smoke", 11, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed by fire, minecarts with
											 * furnace and blazes:
											 * <ul>
											 * <li>It looks like a large gray
											 * cloud
											 * <li>The speed value influences
											 * the velocity at which the
											 * particle flies off
	...
	SMOKE_LARGE("largesmoke", 12, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed when splash potions or
											 * bottles o' enchanting hit
											 * something:
											 * <ul>
											 * <li>It looks like a white swirl
											 * <li>The speed value causes the
											 * particle to only move upwards
											 * when set to 0
											 * <li>Only the motion on the y-axis
											 * can be controlled, the motion on
											 * the x- and z-axis are multiplied
											 * by 0.1 when setting the values to
											 * 0
											 * </ul>
											 */
	SPELL("spell", 13, -1), /**
							 * A particle effect which is displayed when instant
							 * splash potions hit something:
							 * <ul>
							 * <li>It looks like a white cross
							 * <li>The speed value causes the particle to only
							 * move upwards when set to 0
							 * <li>Only the motion on the y-axis can be
							 * controlled, the motion on the x- and z-axis are
							 * multiplied by 0.1 when setting the values to 0
							 * </ul>
							 */
	SPELL_INSTANT("instantSpell", 14,
			-1), /**
					 * A particle effect which is displayed by entities with
					 * active potion effects:
					 * <ul>
					 * <li>It looks like a colored swirl
					 * <li>The speed value causes the particle to be colored
					 * black when set to 0
					 * <li>The particle color gets lighter when increasing the
					 * speed and darker when decreasing the speed
					 * </ul>
					 */
	SPELL_MOB("mobSpell", 15, -1,
			ParticleProperty.COLORABLE), /**
											 * A particle effect which is
											 * displayed by entities with active
											 * potion effects applied through a
											 * beacon:
											 * <ul>
											 * <li>It looks like a transparent
											 * colored swirl
											 * <li>The speed value causes the
											 * particle to be always colored
											 * black when set to 0
											 * <li>The particle color gets
											 * lighter when increasing the speed
											 * and darker when decreasing the
											 * speed
											 * </ul>
											 */
	...
	SPELL_WITCH("witchMagic", 17, -1), /**
										 * A particle effect which is displayed
										 * by blocks beneath a water source:
										 * <ul>
										 * <li>It looks like a blue drip
										 * <li>The speed value has no influence
										 * on this particle effect
										 * </ul>
										 */
	DRIP_WATER("dripWater", 18, -1), /**
										 * A particle effect which is displayed
										 * by blocks beneath a lava source:
										 * <ul>
	...
	DRIP_LAVA("dripLava", 19, -1), /**
									 * A particle effect which is displayed when
									 * attacking a villager in a village:
									 * <ul>
									 * <li>It looks like a cracked gray heart
									 * <li>The speed value has no influence on
									 * this particle effect
									 * </ul>
									 */
	VILLAGER_ANGRY("angryVillager", 20,
			-1), /**
					 * A particle effect which is displayed when using bone meal
					 * and trading with a villager in a village:
					 * <ul>
	...
	TOWN_AURA("townaura", 22, -1,
			ParticleProperty.DIRECTIONAL), /**
											 ...
											 * <li>The speed value causes the
											 * particle to be colored green when
											 * set to 0
											 * </ul>
											 */
	NOTE("note", 23, -1,
			ParticleProperty.COLORABLE), /**
											 * A particle effect which is
											 * displayed by nether portals,
											 * endermen, ender pearls, eyes of
											 * ender, ender chests and dragon
											 * eggs:
											 * <ul>
											 * <li>It looks like a purple cloud
											 * <li>The speed value influences
											 * the spread of this particle
											 * effect
											 * </ul>
											 */
	PORTAL("portal", 24, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed by enchantment tables
											 * which are nearby bookshelves:
											 * <ul>
											 * <li>It looks like a cryptic white
											 * letter
											 * <li>The speed value influences
											 * the spread of this particle
											 * effect
	...
	ENCHANTMENT_TABLE("enchantmenttable", 25, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed by torches, active
											 * furnaces, magma cubes and monster
											 * spawners:
											 * <ul>
	...
	LAVA("lava", 27, -1), /**
							 ...
							 * <li>The speed value has no influence on this
							 * particle effect
							 * </ul>
							 */
	...
	CLOUD("cloud", 29, -1,
			ParticleProperty.DIRECTIONAL), /**
											 * A particle effect which is
											 * displayed by redstone ore,
											 * powered redstone, redstone
											 * torches and redstone repeaters:
											 * <ul>
											 * <li>It looks like a tiny colored
											 * cloud
											 * <li>The speed value causes the
											 * particle to be colored red when
											 * set to 0
											 * </ul>
											 */
	REDSTONE("reddust", 30, -1,
			ParticleProperty.COLORABLE), /**
											 * A particle effect which is
											 * displayed when snowballs hit a
											 * block:
											 * <ul>
											 * <li>It looks like a little piece
											 * with the snowball texture
											 * <li>The speed value has no
											 * influence on this particle effect
											 * </ul>
											 */
	...
	SNOW_SHOVEL("snowshovel", 32, -1,
			ParticleProperty.DIRECTIONAL), /**
											 ...
											 * displayed by slimes:
											 * <ul>
											 * <li>It looks like a tiny part of
											 * the slimeball icon
											 * <li>The speed value has no
											 * influence on this particle effect
											 * </ul>
	...
	BARRIER("barrier", 35,
			8), /**
				 * A particle effect which is displayed when breaking a tool or
				 * eggs hit a block:
				 * <ul>
	...
	ITEM_CRACK("iconcrack", 36, -1, ParticleProperty.DIRECTIONAL,
			ParticleProperty.REQUIRES_DATA), /**
												 * A particle effect which is
												 * displayed when breaking
												 * blocks or sprinting:
												 * <ul>
												 * <li>It looks like a little
												 * piece with a block texture
												 * <li>The speed value has no
												 * influence on this particle
												 * effect
												 * </ul>
	...
	BLOCK_DUST("blockdust", 38, 7, ParticleProperty.DIRECTIONAL,
			ParticleProperty.REQUIRES_DATA), /**
												 * A particle effect which is
												 * displayed when rain hits the
												 * ground:
												 * <ul>
												 * <li>It looks like a blue
												 * droplet
												 * <li>The speed value has no
												 * influence on this particle
												 * effect
	...
	ITEM_TAKE("take", 40,
			8), /**
				 ...
				 * <li>The offset values have no influence on this particle
				 * effect
				 * </ul>
				 */
	...
	public static enum ParticleProperty {
		 ...
		 */
		REQUIRES_WATER, /**
						 * The particle effect requires block or item data to be
						 * displayed
						 */
		REQUIRES_DATA, /**
						 * The particle effect uses the offsets as direction
						 * values
						 */
		DIRECTIONAL, /**
